...
from craftax.fabrax.game_logic import calculate_light_level, get_distance_map
from craftax.fabrax.envs.craftax_state import EnvState, Inventory, Mobs
...
def generate_world(rng, params, static_params):
    ...
    iron_map = jnp.logical_and(
        map == BlockType.STONE.value,
        jax.random.uniform(_rng, static_params.map_size) < 0.02,
    )
    ...
    # Copper ore (requires stone pickaxe, more common than iron)
    rng, _rng = jax.random.split(rng)
    copper_map = jnp.logical_and(
        map == BlockType.STONE.value,
        jax.random.uniform(_rng, static_params.map_size) < 0.025,  # Slightly more common than iron
    )
    map = jnp.where(copper_map, BlockType.COPPER.value, map)

    # Tin ore (requires stone pickaxe, rarer than copper)
    rng, _rng = jax.random.split(rng)
    tin_map = jnp.logical_and(
        map == BlockType.STONE.value,
        jax.random.uniform(_rng, static_params.map_size) < 0.02,  # Rarer than copper
    )
    map = jnp.where(tin_map, BlockType.TIN.value, map)

    # Limestone (requires wood pickaxe, common in stone areas)
    rng, _rng = jax.random.split(rng)
    limestone_map = jnp.logical_and(
        map == BlockType.STONE.value,
        jax.random.uniform(_rng, static_params.map_size) < 0.02,  # More common, needed for early chemistry
    )
    map = jnp.where(limestone_map, BlockType.LIMESTONE.value, map)

    # Clay near water (replaces some sand)
    rng, _rng = jax.random.split(rng)
    clay_map = jnp.logical_and(
        map == BlockType.SAND.value,
        jax.random.uniform(_rng, static_params.map_size) < 0.1,  # 10% of sand becomes clay
    )
    map = jnp.where(clay_map, BlockType.CLAY.value, map)
    ...
    state = EnvState(
        ...
        station_map=jnp.zeros(static_params.map_size, dtype=bool),  # Track station placements
        player_buffs=jnp.zeros((2,), dtype=jnp.int32),  # [stoneskin_duration, ...]
        tool_hardened=jnp.zeros((4,), dtype=bool),  # [steel_pick, bronze_pick, steel_sword, bronze_sword]
        furnace_speed_map=jnp.zeros(static_params.map_size, dtype=jnp.float32),  # Speed bonuses
    )
