#!/usr/bin/env python3
"""
Retry asset generation script for specific failed Fabrax textures.
Uses OpenAI GPT-4 with image generation to recreate problematic assets.
"""
...
def generate_asset(asset_name, asset_config, reference_assets_dir, output_dir):
    ...
    print(f"🎨 Regenerating {asset_name}.png using reference {asset_config['reference_asset']}...")

    try:
        # Encode reference image
        reference_b64 = encode_image_to_base64(reference_path)

        # Create enhanced prompt with very explicit pixel art instructions
        full_prompt = f"""{asset_config['prompt']}

CRITICAL: This image will be automatically downscaled to 16x16 pixels after generation, so:
- Use LARGE chunky pixels like the reference image
- NO fine details or smooth gradients
- NO anti-aliasing or blur
- Simple, bold colors only
- Copy the exact blocky pixel art aesthetic of the reference
- Think classic 1990s video game sprites
- The final result must look good when shrunk to 16x16 pixels"""
        ...
        print(f"✅ Successfully regenerated and saved {asset_name}.png")
        ...
        print(f"❌ Failed to regenerate {asset_name}: {str(e)}")
        return False

def main():
    """Main function to regenerate failed assets."""

    # Paths
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(script_dir, "assets_config_retry.yaml")
    reference_assets_dir = os.path.join(script_dir, "../../craftax_classic/assets")
    output_dir = os.path.join(script_dir, "assets_generated_retry")

    # Create output directory if it doesn't exist
    os.makedirs(output_dir, exist_ok=True)

    # Load configuration
    print("📋 Loading retry asset configuration...")
    ...
    print(f"🔄 Retrying generation of {total_assets} problematic assets...")
    print("=" * 60)

    # Generate each asset
    for asset_name, asset_config in assets.items():
        print(f"\n[{generated_count + failed_count + 1}/{total_assets}] Regenerating {asset_name}...")
    ...
    # Summary
    print("\n" + "=" * 60)
    print("📊 RETRY GENERATION SUMMARY")
    print("=" * 60)
    print(f"✅ Successfully regenerated: {generated_count}")
    print(f"❌ Failed: {failed_count}")
    print(f"📈 Total processed: {generated_count + failed_count}")

    if failed_count == 0:
        print("\n🎉 All problematic assets regenerated successfully!")
    else:
        print(f"\n⚠️  {failed_count} assets still failed to generate. Check errors above.")
        return 1
